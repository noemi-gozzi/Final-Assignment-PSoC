...
uint8_t EEPROM_readStatus() {
	...
	return SPI_Interface_ReadByte(SPI_EEPROM_RDSR, EEPROM_CS);
...
void EEPROM_writeEnable() {
	...
	SPI_Interface_tradeByte(SPI_EEPROM_WREN, EEPROM_CS);
...
uint8_t EEPROM_readByte(uint16_t addr) {
	...
	SPI_Interface_Multi_RW(dataTX, 3, &dataRX, 1, EEPROM_CS);
...
void EEPROM_writeByte(uint16_t addr, uint8_t dataByte) {
	...
	SPI_Interface_Multi_RW(dataTX, 4, &temp, 0, EEPROM_CS);
...
void EEPROM_readPage(uint16_t addr, uint8_t* dataRX, uint8_t nBytes) {
	...
	/* Read the nBytes */
	SPI_Interface_Multi_RW(dataTX, 3, dataRX, nBytes, EEPROM_CS);
...
void EEPROM_writePage(uint16_t addr, uint8_t* data, uint8_t nBytes) {
    ...
    */
	...
	SPI_Interface_Multi_RW(dataTX, 3+nBytes, &temp, 0, EEPROM_CS);
